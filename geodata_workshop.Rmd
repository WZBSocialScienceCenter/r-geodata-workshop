...
<li>Making plots publication-ready</li>
</ol>
...
- but: R gives you several advantages over these tools
    ...
    - also supports geo-spatial analysis (distances, clustering, regional densities, etc.)
...
## Excercises 2 and 3 {.even-smaller}
...
- if your points are too near to each other on a global scale:
    - consider filtering the "worldmap" dataset to contain only the country you need to plotting
    - or: restrict the display window (see hints in handout)

**3. Visualizing WZB partner institutions on a world map**

- load the CSV file `wzb_partners_data.csv` and the data for the world map
...
nutsrg123 <- lapply(1:3, function(lvl) {
    nutsrg <- read_sf(sprintf('excercises/data/nutsrg_%d.json', lvl))
    nutsrg$level <- lvl
    st_crs(nutsrg) <- 3857
    nutsrg
})
