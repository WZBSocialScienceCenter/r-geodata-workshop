...
- load the CSV file `bln_plr_sozind_data.csv` from folder `4_berlin_sozind`
    - make sure that the variable `SCHLUESSEL` is loaded as character string, **not** as integer (use `colClasses = c('SCHLUESSEL' = 'character')` in `read.csv()`)
    - these are the variables in the dataset:<br>
        STATUS1: Unemployment rate 2016 in percent
        STATUS2: Long term unemployment rate 2016 in percent<br>
        STATUS3: Pct. of households that obtain social support ("Hartz IV") 2016<br>
        STATUS4: Portion of children under 15 living in household that obtains social support ("Hartz IV") 2016<br>
        DYNAMO1 to 4: Change in the above indicators from the previous year
- optional: have a look at `bln_plr.geojson` with QGIS
- load the spatial dataset `bln_plr.geojson` containing the Berlin "Planungsraum" regions
    - make sure to set the CRS: `st_crs(<DATASET>) <- 25833`
- join both datasets so that you obtain a combined spatial dataset with the social indicators from `bln_plr_sozind_data.csv` for their respective  regions from `bln_plr.geojson`
- make a simple choropleth map with a social indicator of your choice
- create a variable with discrete bins from a social indicator of your choice and use this to make a more useful choropleth map
...
The [Google Maps API](https://cloud.google.com/maps-platform/) is a web service that allows programmatic geocoding or reverse geocoding (among other things). But simply, this means:
